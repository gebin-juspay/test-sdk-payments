...
#import <TestPaymentsServices/TestPaymentsServices.h>

__weak static TestPaymentsServices *_hyperServicesReference;
...
RCT_EXPORT_METHOD(preFetch:(NSString *)data) {
    if (data && data.length>0) {
        @try {
            NSDictionary *jsonData = [TestSDKReact stringToDictionary:data];
            if (jsonData && [jsonData isKindOfClass:[NSDictionary class]] && jsonData.allKeys.count>0) {
                [TestPaymentsServices preFetch:jsonData];
            } else {
...
RCT_EXPORT_METHOD(createHyperServices) {
    if (self.hyperInstance == NULL) {
        self.hyperInstance = [[TestPaymentServices alloc] init];
    ...
    if (data && data.length>0) {
        @try {
            ...
            if (jsonData && [jsonData isKindOfClass:[NSDictionary class]] && jsonData.allKeys.count>0) {
                ...
                __weak TestSDKReact *weakSelf = self;
...
{
    TestPaymentsServices *hyperServicesInstance = _hyperServicesReference;
